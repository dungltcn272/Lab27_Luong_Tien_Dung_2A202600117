...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=decision,
        reason=f"Routed to {decision} (confidence={c:.0%})",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_human_approval(state):
    ...
    # Audit BEFORE the interrupt — no human has responded yet
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
    ...
    # Audit AFTER resume — reviewer has responded
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="human_approval",
        confidence=a.confidence,
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision=action,
        reason=f"Comment {'posted' if action == 'committed' else 'skipped'}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"Auto-approved with confidence {a.confidence:.0%}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    # Audit BEFORE the interrupt — reviewer hasn't answered yet
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
    ...
    # Audit AFTER resume — reviewer has answered
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
