    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If your confidence is below 60%, you MUST populate escalation_questions "
                "with 2–4 specific, context-rich questions that reference the exact file "
                "and section in the diff where you are uncertain."
            )},
...
def node_escalate(state: ReviewState) -> dict:
    ...
    # Call interrupt with escalation payload containing the LLM's questions
    answers = interrupt({
        "kind": "escalation",
        "pr_url": state["pr_url"],
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": "Refine the code review using the reviewer's answers to produce an improved, more confident assessment."},
            {"role": "user", "content": (
...
def build_graph():
    ...
    # Wire escalate → synthesize → commit (commit already → END)
