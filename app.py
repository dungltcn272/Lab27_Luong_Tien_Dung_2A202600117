...
from common.db import replay_events
...
with st.sidebar:
    ...
    try:
        ...
        async def _list_threads():
            ...
            async with db_conn() as conn:
                async with conn.execute(
                    "SELECT thread_id, pr_url, MAX(risk_level) as worst_risk, "
                    "MAX(action) as last_action, MAX(timestamp) as last_ts "
                    "FROM audit_events GROUP BY thread_id ORDER BY last_ts DESC LIMIT 10"
                ) as cur:
                    return [dict(r) for r in await cur.fetchall()]
        threads = asyncio.run(_list_threads())
        if threads:
            for t in threads:
                label = f"{t['thread_id'][:8]}… | {t.get('worst_risk', '?')} | {t.get('last_action', '?')}"
        ...
        st.caption("No audit data available yet.")
...
def render_escalation_card(payload: dict) -> dict | None:
    ...
    with st.form("escalation"):
        ...
        for i, q in enumerate(payload.get("questions", [])):
            answers[q] = st.text_input(f"Q{i+1}: {q}", key=f"esc_{i}")
        submitted_esc = st.form_submit_button("Submit answers")
    if submitted_esc:
        return answers
    return None
